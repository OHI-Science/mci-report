...
avg <- df%>%
       dplyr::select(pressure,rgn,mean)%>%
        unique()
...
ggplot(avg) + geom_bar(aes(y = mean, x = reorder(rgn,-mean),fill = factor(pressure)), stat = "identity")+
...
ggplot(avg_noclim) + geom_bar(aes(y = mean, x = reorder(rgn,-mean), fill = factor(pressure)), stat = "identity")+
  theme(axis.text.x = element_text(angle = 70, hjust = 1))+
  labs(x = "Region",
       y = "Mean Impact Score",
